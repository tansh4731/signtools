...
const conf = {
  data: {
    ...
    date: '2017-01',
    today: '',
    stateShow: 0, //0-unsign  1-sign 2-时间未到
    week: ['日', '一', '二', '三', '四', '五', '六'],
    calendar: {
      first: [],
      second: [],
  ...
  onLoad() {
    this.setData({
      am1SignToSeconds: 8 * 60 * 60 + 45 * 60 + 0,
      am2SignToSeconds: 12 * 60 * 60 + 0 * 60 + 0,
      pm1SignToSeconds: 13 * 60 * 60 + 30 * 60 + 0,
      pm2SignToSeconds: 17 * 60 * 60 + 45 * 60 + 0,
    })
  },
  ...
  getTimeRecordsByDay(year, month, day) {
    var timeRecords = [];

    for (var index in this.data.records) {
      var record = this.data.records[index];
      var info = util.getDateByTimeNumber(record.signTime);
  ...
  isDayOk(year, month, day) {
    var timeRecords = this.getTimeRecordsByDay(year, month, day);

    var fst = false;
    var sec = false;
    var thrd = false;

    for (var index in timeRecords) {
      var record = timeRecords[index];
      var tmp_seconds = record.hour * 60 * 60 + record.minutes * 60 + record.seconds;

      if (tmp_seconds <= this.data.am1SignToSeconds) {
        fst = true;
      }

      if (tmp_seconds >= this.data.am2SignToSeconds && tmp_seconds <= this.data.pm1SignToSeconds) {
        sec = true;
      }

      if (tmp_seconds >= this.data.pm2SignToSeconds) {
        thrd = true;
      }
    }
  ...
  bindDayTap(e) {
    var timeRecords = this.getTimeRecordsByDay(e.currentTarget.dataset.year, e.currentTarget.dataset.month, e.currentTarget.dataset.day);
    ...
    wx.navigateTo({
      url: 'detail?current=' + JSON.stringify(param),
    })
  ...
  generateDays(year, month, daysNum, option = {
    ...
    let todayMonth = this.formatMonth(todayDate.getMonth() + 1)
    let todyaYear = todayDate.getFullYear()
    let todyDay = this.formatDay(todayDate.getDate())
    let tmpToday = `${todyaYear}-${todayMonth}-${todyDay}`
    ...
    for (let i = option.startNum; i <= daysNum; i++) {
      ...
      if (option.notCurrent || passDay || parseInt(todayMonth) < parseInt(month)) {
        stateShow = 2
